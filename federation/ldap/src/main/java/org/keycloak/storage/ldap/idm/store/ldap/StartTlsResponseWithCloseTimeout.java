 ...
 * for the peer's {@code close_notify} ends in a {@link SocketTimeoutException} rather than hanging. The timeout is
 * only ever set on a socket that is already being torn down, so it cannot cut short a connection still in use.
 */
