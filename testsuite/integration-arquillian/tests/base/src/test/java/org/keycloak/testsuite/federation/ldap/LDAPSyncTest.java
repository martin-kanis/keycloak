...
//    }
    ...
    @Test
    public void test07LDAPGroupSync() {
        testingClient.server().run(session -> {
            LDAPTestContext ctx = LDAPTestContext.init(session);
            RealmModel appRealm = ctx.getRealm();
        ...
        testingClient.server().run(session -> {
            ...
            System.out.println(group1Loaded.toString());

            LDAPDn parentDn = group1Loaded.getDn().getParentDn();
            parentDn.addFirst("cn", "group2");
            group1Loaded.setDn(parentDn);
