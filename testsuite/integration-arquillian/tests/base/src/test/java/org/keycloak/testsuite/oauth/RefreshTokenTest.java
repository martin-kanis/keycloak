...
//            UserInfo userInfo = oauth.doUserInfoRequest(response1.getAccessToken());
    ...
    private int countSessions(final String userSessionId, final String clientId, final String clientSessionId) {
        return testingClient.server().fetch(session -> {
            RealmModel realmModel = session.realms().getRealmByName("test");
            ClientModel clientModel = realmModel.getClientByClientId(clientId);
            UserSessionModel userSession = session.sessions().getUserSession(realmModel, userSessionId);
    ...
    public void refreshTokenUserSessionMaxLifespan() throws Exception {
        ...
        try (Closeable realmUpdater = new RealmAttributeUpdater(realmResource)
            ...
            RealmManager.realm(realmResource).ssoSessionMaxLifespan(3600);
            RealmManager.realm(realmResource).ssoSessionIdleTimeout(7200);
            ...
            assertEquals(2, countSessions(sessionId, loginEvent.getClientId(), clientSessionId));
            ...
            assertEquals(0, countSessions(sessionId, loginEvent.getClientId(), clientSessionId));
        } finally {
    ...
    public void refreshTokenUserClientMaxLifespanSmallerThanSession() throws Exception {
        ...
        try (Closeable realmUpdater = new RealmAttributeUpdater(realmResource)
            ...
            assertNull(tokenResponse.getRefreshToken());
            events.expect(EventType.REFRESH_TOKEN).error(Errors.INVALID_TOKEN).user((String) null).assertEvent();
            assertEquals(1, countSessions(sessionId, loginEvent.getClientId(), clientSessionId));
    ...
    public void refreshTokenClientSessionMaxLifespan() throws Exception {
        ...
        try {
            ...
            tokenResponse = oauth.doRefreshTokenRequest(tokenResponse.getRefreshToken(), "password");
            assertTrue("Invalid RefreshExpiresIn", 0 <= tokenResponse.getRefreshExpiresIn() && tokenResponse.getRefreshExpiresIn() <= 5);

            setTimeOffset(1);

            tokenResponse = oauth.doRefreshTokenRequest(tokenResponse.getRefreshToken(), "password");
            assertTrue("Invalid RefreshExpiresIn", 0 <= tokenResponse.getRefreshExpiresIn() && tokenResponse.getRefreshExpiresIn() <= 4);
            ...
            tokenResponse = oauth.doRefreshTokenRequest(tokenResponse.getRefreshToken(), "password");
            assertEquals(200, tokenResponse.getStatusCode());
            assertTrue("Invalid RefreshExpiresIn", 0 <= tokenResponse.getRefreshExpiresIn() && tokenResponse.getRefreshExpiresIn() <= 3);
